...
module.exports = (function () {
  const tableMixContainer = function () {
  ...
  const createSpectrum = function () {

  };

  const trackUi = function (container, title, type) {
    const content = $(`<div class="table-cell" id="${title}">\
				<div class="table">\
    ...
    const spectrum = content.find(".spectrum canvas");
    spectrum.drawSpectrum = drawSpectrum;

    const tag = content.find("audio").length ? content.find("audio") : content.find("video");

    const playPause = content.find(".play i.fa");
  ...
  const waiUi = function (container) {
  ...
  waiUi.prototype.addTrack = function (track, name) {
    ...
    const dom = trackUi.call(this, this.tracksContainer, name, mediaType);
